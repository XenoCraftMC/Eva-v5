...
const levelUpMessages = require('/app/xiao/assets/json/levelUpMessages.json');
...
class PonyLevels {
    ...
    setGuildMemberExp(member, exp, msg) {
        return new Promise(async (fulfill, reject) => {
            try {
    ...
    giveGuildUserExp(member, message) {
      //console.log(message.guild.id)
       ...
       if (moment().diff(member.timeout || 0) < 0)
        ...
        return new Promise(async (fulfill, reject) => {
            try {
                const oldExp = (await this.getGuildMemberExp(member, message)).exp;
                ...
                await this.setGuildMemberExp(member, newExp, message);
    ...
    updateUserRoles(member, message, fix, args) {
        return new Promise(async (fulfill, reject) => {
            try {
                const exp = (await this.getGuildMemberExp(member, message)).exp;
              ...
              //const lvl = args[0]
                ...
                if (await message.guild.settings.get('message', true) && !fix)
                    await message.channel.send({embed: PonyUtils.generateSuccessEmbed(message, 'Level Up! :arrow_up:', `Congratulations ${member}, just leveled up!`, lvl)});
