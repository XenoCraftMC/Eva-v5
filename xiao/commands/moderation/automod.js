...
module.exports = class AutomodCommand extends Command {
  constructor (client) {
    super(client, {
      ...
      group: 'moderation',
      aliases: ['botmod', 'skynetmod'],
      description: 'General toggle for all automod features (badwords, caps, duptext, emojis, invites, links, mentions, slowmode)',
  ...
  run (msg, {option, rolesx}) {
    ...
    const automodEmbed = new MessageEmbed(),
      ...
      options = {
        enabled: option,
        filterroles: rolesx ? rolesx.map(r => r.id) : []
      };
    ...
    automodEmbed
      ...
      ${rolesx ? `Roles that are exempted from automod: ${rolesx.map(val => `\`${val.name}\``).join(', ')}` : null}
    ...
    if (msg.guild.settings.get('modlogs', true)) {
      ...
      modlogChannel && msg.guild.settings.get('modlogs', false) ? msg.guild.channels.cache.get(modlogChannel).send('', {embed: automodEmbed}) : null;
    }
