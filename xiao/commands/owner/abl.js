...
module.exports = class ABLCommand extends Command {
  ...
  async run(message, args) {
    let blacklist = JSON.parse(fs.readFileSync(path.join(__dirname, '../../bin/blacklist.json'), 'utf8'));
    if (args.type.toLowerCase() === 'guild') {
      ...
      fs.writeFile(path.join(__dirname, '../../bin/blacklist.json'), JSON.stringify(blacklist, null, 2), err => {
        if (err) {
