...
module.exports = class IamCommand extends Command {
  ...
  async run (msg, {role, roleNames = []}) {
    try {
      ...
      roles.forEach(r => roleNames.push(msg.guild.roles.cache.get(r).name));
    ...
    This message will only show up this one time and never again after this so if you desire to set up mod logs make sure to do so now.`);
        ...
        modlogChannel && msg.guild.settings.get('modlogs', false) ? msg.guild.channels.cache.get(modlogChannel).send('', {embed: roleAddEmbed}) : null;
      }
