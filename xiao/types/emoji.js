...
class EmojiArgumentType extends ArgumentType {
	...
	validate(value, msg) {
		...
		let emojis = msg.guild.emojis.cache.filterArray(nameFilterInexact(search));
	...
	parse(value, msg) {
		...
		const search = value.toLowerCase();
		const emojis = msg.guild.emojis.cache.filterArray(nameFilterInexact(search));
		if (!emojis.length) return null;
		if (emojis.length === 1) return emojis[0];
		const exactEmojis = emojis.cache.filterArray(nameFilterExact(search));
