...
const checkReminders = async (client, msg) => {
  ...
  try {
    ...
    if (query === undefined || query.length == 0) {
      ...
      for (const row in query) {
        ...
        if (dura.asMinutes() <= 0) {
          const user = await client.users.cache.get(query[row].userID);
          if (user == "init") {
          } else {
            user.send({
              embed: {
                ...
                author: {
                  name: "Quantum Reminders",
                  iconURL: client.user.displayAvatarURL({ format: "png" })
                },
                thumbnail: {
                  url: "https://favna.xyz/images/ribbonhost/reminders.png"
                }
              }
            });
            conn
    ...
    if (/(?:no such table)/i.test(err.toString())) {
      ...
      conn
        .prepare(
          `CREATE TABLE IF NOT EXISTS "reminders" (userID TEXT , remindTime TEXT, remindText TEXT);`
        )
        .run();
      conn
        .prepare(
      ...
      client.channels.cache.get(process.env.ribbonlogchannel).send(stripIndents`
      ...
      for (const row in rows) {
        ...
        if (dura.asMinutes() <= 0) {
          const guild = client.guilds.get(tables[table].name),
            channel = guild.channels.cache.get(rows[row].channel),
          ...
          timerEmbed
            ...
            That is: ${moment
              .duration(
                moment(rows[row].datetime).diff(moment(), "days"),
                "days"
              )
          ...
          if (moment(rows[row].datetime).diff(new Date(), "hours") >= 24) {
            conn
              .prepare(
                `UPDATE "${tables[table].name}" SET lastsend=$lastsend WHERE id=$id;`
              )
              .run({
                id: rows[row].id,
            ...
            conn
              .prepare(`DELETE FROM "${tables[table].name}" WHERE id=$id;`)
              .run({ id: rows[row].id });
...
const guildAdd = async (client, guild) => {
  try {
    const avatar = await Jimp.read(
      ...
      fontMedium = await Jimp.loadFont(
    ...
    newGuildEmbed
      ...
      .setColor("#80F31F")
      .setTitle("QuantumProjects is here!")
      .setDescription(
        stripIndents`
...
const guildLeave = (client, guild) => {
  ...
  const casinoConn = new Database(
    ...
    pastasConn = new Database(
      path.join(__dirname, "../data/databases/pastas.sqlite3")
    ),
    timerConn = new Database(
      path.join(__dirname, "../data/databases/timers.sqlite3")
    ),
    warningsConn = new Database(
      path.join(__dirname, "../data/databases/warnings.sqlite3")
    ),
    profileConn = new Database(
      path.join(__dirname, "../data/databases/profile.sqlite")
    ),
    levelsConn = new Database(
      path.join(__dirname, "../data/databases/levels.sqlite3")
    );
  ...
  try {
    profileConn.exec(`DROP TABLE IF EXISTS "${guild.id}"`);
  } catch (err) {
    client.channels.cache.get(process.env.ribbonlogchannel).send(stripIndents`
      <@${client.owners[0].id}> Failed to purge ${guild.name} (${
      guild.id
  ...
  try {
    levelsConn.exec(`DROP TABLE IF EXISTS "${guild.id}"`);
  } catch (err) {
    client.channels.cache.get(process.env.ribbonlogchannel).send(stripIndents`
      <@${client.owners[0].id}> Failed to purge ${guild.name} (${
      guild.id
    }) from the levels database!
      **Time:** ${moment().format("MMMM Do YYYY [at] HH:mm:ss [UTC]Z")}
      **Error Message:** ${err}
      `);
  }
...
const joinMessage = async member => {
  try {
    ...
    canvas.print(
      ...
      160,
      60,
      `you are the ${ordinal(member.guild.memberCount)} member`.toUpperCase()
    );
    ...
    newMemberEmbed
      ...
      .setTitle("NEW MEMBER!")
      .setDescription(
        `Please give a warm welcome to __**${member.displayName}**__  (\`${member.id}\`)`
      )
      .setImage("attachment://joinimg.png");

    return member.guild.channels.cache
      .get(member.guild.settings.get("joinmsgchannel"))
      .send(`welcome <@${member.id}> 🎗️!`, { embed: newMemberEmbed });
  } catch (err) {
    return null;
  }
};
...
const leaveMessage = async member => {
  try {
    ...
    leaveMemberEmbed
      ...
      .setDescription(
        `You will be missed __**${member.displayName}**__ (\`${member.id}\`)`
      )
      .setImage("attachment://leaveimg.png");

    return member.guild.channels.cache
      .get(member.guild.settings.get("leavemsgchannel"))
      .send("", { embed: leaveMemberEmbed });
  } catch (err) {
    return null;
  }
};
...
const lotto = client => {
  ...
  try {
    ...
    for (const table in tables) {
      ...
      conn
        .prepare(
          `UPDATE "${tables[table].name}" SET balance=$balance WHERE userID="${rows[winner].userID}"`
        )
      ...
      const defaultChannel = client.guilds.get(tables[table].name)
        ...
        winnerLastMessage = client.guilds
          .get(tables[table].name)
          .members.get(rows[winner].userID).lastMessageChannelID,
        winnerLastMessageChannel = winnerLastMessage
          ? client.guilds
              .get(tables[table].name)
              .channels.cache.get(winnerLastMessage)
          : null,
        winnerLastMessageChannelPermitted = winnerLastMessageChannel
      ...
      winnerEmbed
        .setColor("#7CFC00")
        .setDescription(
          `Congratulations <@${rows[winner].userID}>! You won today's random lotto and were granted 2000 chips 🎉!`
        )
        .setAuthor(
          client.guilds.get(tables[table].name).members.get(rows[winner].userID)
            .displayName,
          client.guilds
            .get(tables[table].name)
...
module.exports = {
  ...
  lotto,
  timerMessages,
  voteRewards: function rewards(message, client) {
    const conn = new Database("/app/xiao/data/databases/casino.sqlite3");

    const guilds = client.guilds;
    for (var prop in guilds) {
      var members = prop.members;
      for (var memberd in members) {
        if (memberd.id == message.user) {
          var memguild = memberd.guild.id;
          try {
            const query = conn
              .prepare(`SELECT * FROM "${memguild}" WHERE userID = ?;`)
              .get(message.user);

            conn
              .prepare(`INSERT INTO "${memguild}" VALUES ($userid, $balance);`)
              .run({
                userid: memberd.id,
                balance: query.balance + 1000
              });
          } catch (err) {
            if (/(?:no such table)/i.test(err.toString())) {
              conn
                .prepare(
                  `CREATE TABLE IF NOT EXISTS "${memguild}" (userID TEXT PRIMARY KEY, balance INTEGER, lasttopup TEXT, ledger INTEGER);`
                )
                .run();

              conn
                .prepare(
                  `INSERT INTO "${memguild}" VALUES ($userid, $balance, $date, $ledger);`
                )
                .run({
                  userid: memberd.id,
                  balance: "500",
