...
//	const dataDir = path.resolve(`${__dirname}${path.sep}dashboard`);
  const dataDir = `/app/xiao/dashboard/`
  console.log(dataDir)
  ...
  console.log(templateDir)
	...
	*/
	...
	client.callbackURL = `${protocol}${client.config.dashboard.domain}`;
	console.log('log', `Callback URL: ${client.callbackURL}`, 'INFO');
	passport.use(new Strategy({
		clientID: 478504038732791838,
	...
	*/
	...
	function checkAdmin(req, res, next) {
		if (req.isAuthenticated() && req.user.id === process.env.OWNERS) return next();
  ...
  fs.readFile(`/app/xiao/dashboard/public/PRIVACY.md`, function(err, data) {
		if (err) {
			console.log(err);
			privacyMD = 'Error';
			return;
		}
		privacyMD = data.toString().replace(/\{\{botName\}\}/g, 'Eva').replace(/\{\{email\}\}/g, client.config.dashboard.legalTemplates.contactEmail);
		if (client.config.dashboard.secure !== 'true') {
  ...
  fs.readFile(`/app/xiao/dashboard/public/TERMS.md`, function(err, data) {
		if (err) {
			console.log(err);
			privacyMD = 'Error';
			return;
		}
		termsMD = data.toString().replace(/\{\{botName\}\}/g, 'Eva').replace(/\{\{email\}\}/g, client.config.dashboard.legalTemplates.contactEmail);
	});
	...
	app.get('/', (req, res) => {
		if (req.isAuthenticated()) {
			res.render(path.resolve(`/app/xiao/dashboard/templates/index.ejs`), {
				bot: client,
				auth: true,
				user: req.user
			});
    ...
    res.render(path.resolve('/app/xiao/dashboard/templates/stats.ejs'), {
			bot: client,
...
//		res.render(path.resolve(`${templateDir}${path.sep}legal.ejs`), {
    res.render(path.resolve(`/app/xiao/dashboard/templates/legal.ejs`), {
			bot: client,
			auth: req.isAuthenticated() ? true : false,
			user: req.isAuthenticated() ? req.user : null,
    ...
    res.render(path.resolve('app/xiao/dashbaord/templates/admin.ejs'), {
			bot: client,
			user: req.user,
			auth: true
		});
    ...
    res.render(path.resolve('/app/xiao/dashboard/templates/dashboard.ejs'), {
			perms: perms,
			bot: client,
			user: req.user,
			auth: true
	...
	app.post('/manage/:guildID', checkAuth, (req, res) => {
		...
		const settings = (guild.id).settings;
    ...
    res.render(path.resolve('/app/xiao/dashboard/template/manage.ejs'), {
			bot: client,
			guild: guild,
			user: req.user,
			auth: true
	...
	app.get('/commands', (req, res) => {
      ...
      res.render(path.resolve('/app/xiao/dashboard/templates/commands.ejs'), {
				bot: client,
				auth: true,
				user: req.user,
				md: md
	...
	client.site = app.listen(client.config.dashboard.port, function() {
		console.log('log', `Dashboard running on port ${client.config.dashboard.port}`, 'INFO');
	}).on('error', (err) => {
